...
import { formatINR } from './contractConstants';
 ...
 *
 * Budget logic lives in backend (GET /contracts/project/{id}/budget-summary).
 * This component just displays the result and signals the parent to block submission.
 */
export default function BudgetBreakdown({ project, currentValue = 0, onBudgetStatus }) {
  const [summary, setSummary] = useState(null);
  const [loading, setLoading] = useState(false);

  // Fetch budget summary when project changes
  useEffect(() => {
  ...
  // Notify parent whenever value or summary changes
  useEffect(() => {
    ...
    <div className="mt-1 space-y-1">
      {loading ? (
        ...
        <>
          ...
          {/* Live preview when a value is typed */}
          {val > 0 && (
